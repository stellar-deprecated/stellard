...
suite("Offer tests", function() {
  ...
  test("offer create then crossing offer with STR. Negative balance.", function (done) {
      var self = this;
      ...
      async.waterfall([
        ...
        function (callback) {
          ...
          $.remote.transaction()
            .offer_create("alice", "50/USD/mtgox", "150000.0")    // get 50/USD pay 150000/STR
              .once('proposed', function (m) {
                  // console.log("PROPOSED: offer_create: %s", JSON.stringify(m));
...
//        },
  ...
  test("offer create then crossing offer with STR. Reverse order." , function (done) {
      var self = this;
      ...
      async.waterfall([
        ...
        function (callback) {
          ...
          $.remote.transaction()
            .offer_create("bob", "1/USD/mtgox", "4000.0")         // get 1/USD pay 4000/STR : offer pays 4000 STR for 1 USD
              .on('submitted', function (m) {
                  // console.log("PROPOSED: offer_create: %s", JSON.stringify(m));
        ...
        function (callback) {
          ...
          // Pay 1 USD, get 4000 STR.

          $.remote.transaction()
            .offer_create("alice", "150000.0", "50/USD/mtgox")  // get 150,000/STR pay 50/USD : offer pays 1 USD for 3000 STR
              .on('submitted', function (m) {
                  // console.log("PROPOSED: offer_create: %s", JSON.stringify(m));
...
//        },
  ...
  test("offer create then crossing offer with STR.", function (done) {
      var self = this;
      ...
      async.waterfall([
        ...
        function (callback) {
          ...
          $.remote.transaction()
            .offer_create("alice", "150000.0", "50/USD/mtgox")  // pays 1 USD for 3000 STR
              .on('submitted', function (m) {
                  // console.log("PROPOSED: offer_create: %s", JSON.stringify(m));
        ...
        function (callback) {
          ...
          $.remote.transaction()
            .offer_create("bob", "1/USD/mtgox", "4000.0") // pays 4000 STR for 1 USD
              .on('submitted', function (m) {
                  // console.log("PROPOSED: offer_create: %s", JSON.stringify(m));
...
//        },
  ...
  test("offer create then crossing offer with STR with limit override.", function (done) {
      var self = this;
...
//              "bob" : "1000/USD/mtgox"
        ...
        function (callback) {
          ...
          $.remote.transaction()
            .offer_create("alice", "150000.0", "50/USD/mtgox")  // 300 STR = 1 USD
              .on('submitted', function (m) {
                  // console.log("PROPOSED: offer_create: %s", JSON.stringify(m));
  ...
  test("ripple currency conversion : entire offer", function (done) {
    // mtgox in, STR out
      ...
      async.waterfall([
          ...
          function (callback) {
            self.what = "Alice converts USD to STR.";
...
//          },
  ...
  test("ripple currency conversion : in parts", function (done) {
      ...
      async.waterfall([
          ...
          function (callback) {
            self.what = "Alice converts USD to STR should fail due to PartialPayment.";
...
suite("Offer cross currency", function() {
  ...
  test("ripple cross currency payment - start with STR", function (done) {
    // alice --> [STR --> carol --> USD/mtgox] --> bob
      var self = this;
      var seq;
...
//              "alice"   : [ "500" ],
  ...
  test("ripple cross currency payment - end with STR", function (done) {
    // alice --> [USD/mtgox --> carol --> STR] --> bob
      var self = this;
      var seq;
  ...
  test("ripple cross currency bridged payment", function (done) {
    // alice --> [USD/mtgox --> carol --> STR] --> [STR --> dan --> EUR/bitstamp] --> bob
      var self = this;
      var seq_carol;
      var seq_dan;
  ...
  test("offer fee consumes funds", function (done) {
      ...
      async.waterfall([
          function (callback) {
            ...
            starting_STR = reserve_amount.add($.remote.fee_tx(fee_units_for_all_txs))
            // console.log("starting_STR after %s fee units: ",  fee_units_for_all_txs, starting_STR.to_human());

            starting_STR = starting_STR.add(Amount.from_json('100.0'));
            // console.log("starting_STR adding 100 STR to sell", starting_STR.to_human());

            testutils.create_accounts($.remote,
                "root",
                starting_STR.to_json(),
  ...
  test("2x sell exceed limit", function (done) {
      ...
      async.waterfall([
          function (callback) {
            // Provide micro amounts to compensate for fees to make results round nice.
            self.what = "Create accounts.";
            var starting_STR = $.amount_for({
              ledger_entries: 1,
              default_transactions: 2,
              extra: '100.0'
            });
            testutils.create_accounts($.remote, "root", starting_STR, ["alice", "bob", "mtgox"], callback);
          },
          function (callback) {
            self.what = "Set limits.";
          ...
          function (callback) {
            ...
            // Taker pays 200 STR for 100 USD.
            // Selling USD.
            $.remote.transaction()
          ...
          function (callback) {
            ...
            // Taker pays 100 USD for 100 STR.
            // Selling STR.
            // Will sell all 100 STR and get more USD than asked for.
            $.remote.transaction()
