...
namespace stellar
{
    ...
    string LedgerDatabase::getState(StoreStateName stateName) {
        string res;
        string sql = str(boost::format("SELECT State FROM StoreState WHERE StateName = '%s';")
            % getStoreStateName(stateName)
            );
    ...
    void LedgerDatabase::setState(StoreStateName stateName, const string &value) {
    ...
    void LedgerDatabase::endTransaction(bool commit) {
        try {
            mDBCon->getDB()->endTransaction(commit);
        }
        catch (...) {
            mDBCon->getDBLock().unlock();
            throw;
